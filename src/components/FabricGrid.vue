...
export default {
  ...
  setup(props) {
    ...
    const setBeadSize = () => {
      const estimatedSize = Math.floor(canvasWidth.value / store.imgWidth);
      beadSize.value = estimatedSize < 20 ? 20 : estimatedSize;
      console.log("Beadsize", beadSize, canvasWidth.value, store.imgWidth);
    };

    watch(
      () => store.hoveredBead,
      ({ code, name }) => {
        highlightBeads(code + name, store.selectedBead);
      }
    );
    ...
    const highlightBeads = (matchingBead, selectedBead) => {
      //const objs = canvas.getObjects();
      ...
      if (selectedBead) {
        console.log(selectedBead);
        selectedBeads = canvas._objects.filter(
          (obj) => obj.code + obj.name === selectedBead.code + selectedBead.name
        );
        selectedBeads.forEach((item) => {
          item.set("stroke", "yellow");
          item.set("strokeDashArray", [4]);
          canvas.bringToFront(item);
        });
      }
      canvas.renderAll();
